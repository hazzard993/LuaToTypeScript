...
export function getPreviousNode(
  chunk: luaparse.Chunk,
  node: luaparse.Node,
): luaparse.Node | undefined {
...
export function getParameterTParam(
  ...
  const name =
    parameter.type === "Identifier" ? parameter.name : parameter.value;
...
export function getCommentsAsString(
  chunk: luaparse.Chunk,
  node: luaparse.Node,
): string {
  ...
  comments.forEach((comment) => {
    ...
    switch (tagName) {
      ...
      case "@tparam": {
        ...
        availableTags.push(
          ldoc.createTParamTag(name, type, description.join(" ")),
        );
        break;
      }
      case "@return": {
...
export function getMemberExpressionBaseIdentifier(
